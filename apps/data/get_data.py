...
class GetData:
    ...
    def tran_data(self,data):
        ...
        for each_pre in prefix:
            if each_pre in data:
                return Decimal(float(data.strip(each_pre))*prefix[each_pre]).quantize(Decimal("0.00"))
            # if 'K' in data:
    ...
    def tran2human(self,data):
        symbols = ('K','M','G','T','P','E','Z','Y')
        prefix = {'K':1000,'M':1000000,'G':1000000000,'T':1000000000000,'P':1000000000000000,'E':1000000000000000000,'Z':1000000000000000000000,'Y':1000000000000000000000000}
        for k in symbols[::-1]:
            if data >= prefix[k]:
                value = float(data) / prefix[k]
                return '%.2f%s' % (value,k)
        return '%s' % data
    ...
    def get_data(self,values):
        ...
        Q10_Barcode = '{}({}%)'.format(self.tran2human(Q10_Barcode),round(Q10_Barcode/Total_reads*100,2))
        Q20_Barcode = '{}({}%)'.format(self.tran2human(Q20_Barcode),round(Q20_Barcode/Total_reads*100,2))
        Q30_Barcode = '{}({}%)'.format(self.tran2human(Q30_Barcode),round(Q30_Barcode/Total_reads*100,2))
        Q10_UMI = '{}({}%)'.format(self.tran2human(Q10_UMI),round(Q10_UMI/Total_reads*100,2))
        Q20_UMI = '{}({}%)'.format(self.tran2human(Q20_UMI),round(Q20_UMI/Total_reads*100,2))
        Q30_UMI = '{}({}%)'.format(self.tran2human(Q30_UMI),round(Q30_UMI/Total_reads*100,2))
