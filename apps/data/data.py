...
def key_indic(values):
    ...
    if values:
        if len(values) == len(GD.data_dict["1.Filter_and_Map"]["1.1.Adapter_Filter"].keys()):
            return key_indicator(GD.tran2human(DuPlication_Reads),GD.tran2human(Unique_Reads))
        else:
            return [dbc.Card(
                        dbc.CardBody([
                                html.H4(children = [],id = 'raw_reads', className="card-title text-center text-green"),
...
def data_set(values):
    if values:
        ...
        return GD.tran2human(Barcode_Mapping),GD.tran2human(clean_reads),Q20_Barcode,Q30_Barcode,Q30_UMI,Q10_Barcode,Q10_UMI,\
    Q20_UMI,Contour_area,Reads_under_tissue,Number_of_DNB_under_tissue,Ratio,Total_Gene_type,\
    Raw_Reads,Fraction_Reads_in_Spots_Under_Tissue,GD.tran2human(raw_reads),GD.tran2human(Reference_Mapping_reads)
    else:
        return '','','','','','','','','','','','','','','','',''
