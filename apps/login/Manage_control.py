...
def Managelayout():
    ...
    menus = [dbc.Button(id = {'index':'all_order','type':'menus_id'},
        href='/Manager_console#status=all_order',color='link',block=True,
    ...
    for i in statusDict.values():
        if i == '' or i == 'end':
            ...
            single_mennu = dbc.Button(id = {'index':ii,'type':'menus_id'},
                href=f'/Manager_console#status={ii}',color='link',block=True,
    ...
    menus.append(dbc.Button(id = {'index':'end','type':'menus_id'},
        href='/Manager_console#status=end',color='link',block=True,
...
def page_on_status(href_content,page_number,search_order_id,search_order_type,search_start_date,search_end_date,page_size):
    ...
    if status in statusDict:
        ...
        return [class_name,tableShown(status,page_count=page_number,PAGE_SIZE=page_size),page_number]  
    elif search_status in statusDict:
        class_name = []
        statusDict[search_status] = "active text"
        for i in statusDict:
            class_name.append(statusDict[i])
        return [class_name,tableShown('search',search_status,search_order_id,search_order_type,search_start_date,search_end_date,page_count=page_number,PAGE_SIZE=page_size),page_number]
...
def getOrderMenu():
    statusDict = {}
    count=0
    menus=[dbc.Button('All Order', id={"type":"order_menu", 'id':count}, href='#all_order', color='link', block=True)]
    orderstatus = OrderStatus()
    allstatus = orderstatus.getAllStatus()
    for eachstatus in allstatus.iloc:
        statusDict[eachstatus['OrderStatusID']] = eachstatus['OrderStatusName']
    ...
    count += 1
    menus.append(dbc.Button('Finish Order', id={"type":"order_menu", 'id':count},href='#end_order', color='link', block=True))
    return dbc.ButtonGroup(menus, vertical=True, style={'width':'100%'})

@app.callback(
    Output({"type":"order_menu", 'id':ALL},'active'),
    [Input({"type":"order_menu", 'id':ALL},'n_clicks')]
)
def click_menu(menu_n):
    if not dash.callback_context.triggered:
        raise PreventUpdate
    print(dash.callback_context.triggered)
...
#     [State({'type':"menus_id",'index':ALL},'active')]
#     )
# def change_menus_class(menus_id,temp):
#     # 
#     if not dash.callback_context.triggered:
#         raise PreventUpdate
#     print(dash.callback_context.triggered)
